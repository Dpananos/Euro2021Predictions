...
scaled_pts = (pts - mean(pts))/(2*sd(pts))

# Check data quality
teams = sort(unique(c(euro_data$team...1, euro_data$team...3)))
prior_score = scaled_pts[match(ranking_data$team, teams)]
nteams = length(teams)
...
model = cmdstan_model('models/euro_model_prior_scores.stan')
...
goal_diff = function(teamA, teamB){
  ...
  ai = a[, ixa]
  aj = a[, ixb]
  random_outcome_on_sqrt = (ai - aj) + rt(nrow(ai-ai), df)*sigma_y
...
euro_data %>% 
  ...
  geom_point(aes(match, a_obs), color = 'red')+
  coord_flip()
```
