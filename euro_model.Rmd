...
group_a_teams = tibble(team = c('France','Switzerland','Albania','Romania'), group = 'A')
group_b_teams = tibble(team = c('Wales','England','Slovakia','Russia'), group = 'B' )
group_c_teams = tibble(team = c('Germany','Poland','NorthernIreland', 'Ukraine'), group = 'C')
group_d_teams = tibble(team = c('Croatia', 'Spain', 'Turkey', 'Czech'), group = 'D' )
group_e_teams = tibble(team = c('Italy', 'Belgium','Ireland', 'Sweden'), group = 'E' )
group_f_teams = tibble(team = c('Hungary','Iceland','Austria','Portugal'), group = 'F')
...
```{r}
read_csv('predictions/predictions_loss.csv') %>% 
  select(`Draw`, DrawHappened) %>% 
  pull(Draw) %>% hist
```
