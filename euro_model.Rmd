...
bayesplot::ppc_ecdf_overlay(y, yppc)
```
...
```{r}
 fit$draws('yppc') %>% 
  as_draws_df() %>% 
  spread_draws(yppc[i]) %>% 
  summarise(predicted_win_prob = mean(yppc>0)) %>% 
  bind_cols(euro_data) %>% 
  mutate(dif = as.integer((score...4 - score...6)>0),
         match = str_c(team...3, ' v. ', team...5))  -> mod_pred




Metrics::auc(mod_pred$dif, mod_pred$predicted_win_prob)

pROC::roc(mod_pred$dif, mod_pred$predicted_win_prob) %>% 
  pROC::ggroc()
  
  mod_pred %>% 
  ggplot(aes(predicted_win_prob, dif))+
  geom_rug()+
  geom_abline()+
  geom_smooth()
```
